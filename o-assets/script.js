...
function renderRoadmap(data) {
  ...
  const makeCard = (step, idx, c, label, emoji, suffix) => `
            <div class="roadmap-card bg-white/90 dark:bg-slate-800/90 backdrop-blur-sm p-5 rounded-2xl shadow-lg border-2 ${
              c.card
            } cursor-pointer select-none transition-all duration-300 hover:shadow-2xl hover:scale-[1.02] group" onClick="toggleRoadmapDetail(this)">
    ...
    step.date
                ...
                <p class="text-sm text-slate-600 dark:text-slate-400 font-medium leading-relaxed">${convertStr(
                  step.event,
                )}</p>
                ...
                <div class="roadmap-details overflow-hidden transition-all duration-400 ease-in-out" style="max-height:0;opacity:0;">
                    <div class="pt-3 mt-3 border-t border-dashed ${c.card}">
                        <p class="text-sm text-slate-500 dark:text-slate-400 leading-relaxed">${convertStr(
                          step.details || '',
                        )}</p>
                    </div>
                </div>
  ...
  const roadmapHTML = data.roadmap.steps
    .map((step, idx) => {
      ...
      return `
            <div class="roadmap-step" data-step="${idx}" data-reveal="${
        isLeft ? 'fade-right' : 'fade-left'
      }" data-reveal-delay="${Math.min(idx * 100, 400)}">
                <!-- Mobile layout -->
                <div class="flex items-start gap-4 md:hidden">
                    <div class="roadmap-mobile-dot shrink-0 w-12 h-12 rounded-full bg-gradient-to-br ${
                      c.bg
                    } flex items-center justify-center text-xl shadow-lg ring-4 ${
        c.ring
      } ring-offset-2 mt-1 z-10">
                        ${emoji}
                    </div>
                    <div class="flex-1">${makeCard(
                      step,
                      idx,
                      c,
                      label,
                ...
                <div class="hidden md:grid roadmap-grid">
                    ...
                    <div class="roadmap-col-center">
                        <div class="roadmap-dot w-16 h-16 rounded-full bg-gradient-to-br ${
                          c.bg
                        } flex items-center justify-center text-2xl shadow-xl ring-4 ${
        c.ring
      } ring-offset-2 ring-offset-emerald-50 dark:ring-offset-slate-900 cursor-pointer transition-all duration-300 hover:scale-125 hover:ring-offset-4" onClick="toggleRoadmapDetail(this, true)">
                            ${emoji}
                        </div>
  ...
  if (typeof window.scrollRevealObserveAll === 'function')
    window.scrollRevealObserveAll();
...
function renderIndex(data) {
  ...
  if (document.getElementById('hero-title')) {
    ...
    document.getElementById('hero-title').innerHTML = titleText
      .replace(
        'Môn Toán',
        "<span class='text-primary underline decoration-blue-500/30'>môn Toán</span>",
      )
      .replace('Online', `<span class="hero-online-badge">Online</span>`);
    document.getElementById('hero-subtitle').innerHTML = convertStr(
      data.hero.subtitle,
    );
    document.getElementById(
      'hero-btn-trial',
    ).innerHTML = `<span class="material-symbols-outlined">play_circle</span> ${data.hero.buttons.trial.text}`;
    document.getElementById('hero-btn-trial').href =
      data.hero.buttons.trial.link;
    document.getElementById('hero-btn-register-alt').textContent =
      data.hero.buttons.registerNow.text;
    document.getElementById('hero-btn-register-alt').href =
      data.hero.buttons.registerNow.link;
    ...
    if (front && back) {
      ...
      if (
        data.hero &&
        data.hero.list_image &&
        data.hero.list_image.length > 0
      ) {
        allImages = data.hero.list_image.filter(Boolean);
      } else if (data.gallery && data.gallery.categories) {
        allImages = data.gallery.categories
          .flatMap((c) => c.list_image || [])
          .filter(Boolean);
      }
  ...
  if (document.getElementById('rules-title')) {
    ...
    document.getElementById('rule-1-title').textContent =
      data.rules.participants.title;
    document.getElementById('rule-1-desc').innerHTML = convertStr(
      data.rules.participants.content,
    );
    document.getElementById('rule-2-title').textContent =
      data.rules.structure.title;
    document.getElementById('rule-2-desc').innerHTML = convertStr(
      data.rules.structure.content,
    );
    document.getElementById('rule-3-title').textContent = data.rules.time.title;
    document.getElementById('rule-3-desc').innerHTML = convertStr(
      data.rules.time.content,
    );
    if (document.getElementById('rule-4-title')) {
      document.getElementById('rule-4-title').textContent =
        data.rules.attempts.title;
      document.getElementById('rule-4-desc').innerHTML = convertStr(
        data.rules.attempts.content,
      );
    }
  }
  ...
  if (document.getElementById('partners-container') && data.partners) {
    ...
    document.getElementById('partners-desc').textContent =
      data.partners.description;
    document.getElementById('partners-container').innerHTML = data.partners.list
      .map(
        (p) => `
  ...
  if (document.getElementById('experts-container') && data.experts) {
    ...
    document.getElementById('experts-desc').textContent =
      data.experts.description;
    const expertCard = (e) => `
  ...
  if (document.getElementById('gallery-slider-container')) {
    ...
    function renderRandomGallery() {
      ...
      if (!category || !category.list_image || category.list_image.length === 0)
        return;
        ...
        .map((imgSrc, idx) => {
          ...
          return `
                    <div class="absolute inset-0 transition-all duration-1000 ease-in-out shadow-[0_10px_30px_rgba(0,0,0,0.3)] rounded-[2rem] overflow-hidden border-8 border-white dark:border-slate-800 cursor-pointer" 
                         data-lb-idx="${idx}"
                         style="z-index: ${zIndex}; --final-transform: ${transform}; opacity: 0; animation: gallery-slide-up 0.8s ease forwards ${idx *
            3}s;">
                        <img alt="${
                          category.name
                        }" class="w-full h-full object-cover bg-slate-200" src="${imgSrc}"/>
                    </div>
                `;
        })
        .join('');
...
function renderRulesPage(data) {
  if (document.getElementById('page-title'))
    document.getElementById('page-title').textContent = data.rules.title;

  if (document.getElementById('rule-1-title'))
    document.getElementById('rule-1-title').textContent =
      data.rules.participants.title;
  if (document.getElementById('rule-1-desc'))
    document.getElementById('rule-1-desc').innerHTML = convertStr(
      data.rules.participants.content,
    );

  if (document.getElementById('rule-2-title'))
    document.getElementById('rule-2-title').textContent =
      data.rules.structure.title;
  if (document.getElementById('rule-2-desc'))
    document.getElementById('rule-2-desc').innerHTML = convertStr(
      data.rules.structure.content,
    );

  if (document.getElementById('rule-3-title'))
    document.getElementById('rule-3-title').textContent = data.rules.time.title;
  if (document.getElementById('rule-3-desc'))
    document.getElementById('rule-3-desc').innerHTML = convertStr(
      data.rules.time.content,
    );
  ...
  if (document.getElementById('rule-4-title'))
    document.getElementById('rule-4-title').textContent =
      data.rules.attempts.title;
  if (document.getElementById('rule-4-desc'))
    document.getElementById('rule-4-desc').innerHTML = convertStr(
      data.rules.attempts.content,
    );
}
...
function renderGalleryPage(data) {
  if (document.getElementById('page-title'))
    document.getElementById('page-title').textContent = data.gallery.title;
  ...
  function getLbCaption() {
    return document.getElementById('lb-caption');
  }
  function getLbCounter() {
    return document.getElementById('lb-counter');
  }
  function getLbCatTabs() {
    return document.getElementById('lb-cat-tabs');
  }
  let lbImages = [];
  let lbIndex = 0;
  ...
  function renderTabs() {
    ...
    const inactiveClass =
      'bg-white dark:bg-slate-800 text-slate-600 dark:text-slate-300 border border-slate-200 dark:border-slate-700 hover:border-primary dark:hover:border-primary';

    let html = `<button data-cat="all" class="px-6 py-2.5 rounded-xl text-sm font-bold shrink-0 transition-colors ${
      activeCategory === null ? activeClass : inactiveClass
    }">Tất cả</button>`;
    categories.forEach((cat, idx) => {
      html += `<button data-cat="${idx}" class="px-6 py-2.5 rounded-xl text-sm font-bold shrink-0 transition-colors ${
        activeCategory === idx ? activeClass : inactiveClass
      }">${cat.name}</button>`;
    });
  ...
  function getFilteredImages(catIdx) {
    return catIdx === null
      ? categories.flatMap((cat) =>
          (cat.list_image || []).map((img) => ({ name: cat.name, image: img })),
        )
      : (categories[catIdx].list_image || []).map((img) => ({
  ...
  function updateLightbox() {
    ...
    if (lbCatTabs) {
      ...
      let tabsHtml = `<button data-lbcat="all" class="px-3 py-1 rounded-lg text-xs font-bold transition-colors ${
        lbCatIndex === null ? activeClass : inactiveClass
      }">Tất cả</button>`;
      categories.forEach((cat, idx) => {
        tabsHtml += `<button data-lbcat="${idx}" class="px-3 py-1 rounded-lg text-xs font-bold transition-colors ${
          lbCatIndex === idx ? activeClass : inactiveClass
        }">${cat.name}</button>`;
      });
...
function renderVideoSection(data) {
      ...
      (video, idx) => `
        <div class="group rounded-2xl overflow-hidden shadow-lg border border-blue-50 dark:border-slate-700 hover-card-lift bg-white dark:bg-slate-800 cursor-pointer" data-video-idx="${idx}">
            <div class="relative overflow-hidden bg-slate-200 dark:bg-slate-700" style="aspect-ratio:16/9">
                <img alt="${
                  video.title
                }" loading="lazy" class="w-full h-full object-cover group-hover:scale-105 transition-transform duration-700" src="${video.thumb ||
        `https://img.youtube.com/vi/${video.id}/hqdefault.jpg`}"/>
            ...
            <div class="p-4">
                <p class="text-slate-800 dark:text-slate-100 text-sm font-bold leading-snug line-clamp-2">${
                  video.title
                }</p>
            </div>
        </div>
  ...
  if (vmModal && !vmModal.dataset.inited) {
    ...
    vmModal.addEventListener('click', (e) => {
      if (e.target === vmModal) closeVideoModal();
    });
    document.addEventListener('keydown', (e) => {
...
function renderFooter(data) {
  ...
  if (col1) {
    col1.innerHTML = data.footer.column1.details
      .map((d) => `<li class="mb-2">${d}</li>`)
      .join('');
  }

  const col2Links = document.getElementById('footer-col2-links');
  if (col2Links) {
    document.getElementById('footer-col2-title').textContent =
      data.footer.column2.title;
    col2Links.innerHTML = data.footer.column2.links
      .map(
        (l) =>
          `<li><a class="hover:text-primary transition-colors" href="${l.link}">${l.text}</a></li>`,
      )
      .join('');
  }
  ...
  if (col3Links) {
    document.getElementById('footer-col3-title').textContent =
      data.footer.column3.title;
    col3Links.innerHTML = `
  ...
  if (document.getElementById('footer-email'))
    document.getElementById('footer-email').textContent =
      data.footer.column3.details.email;
  if (document.getElementById('footer-phone'))
    document.getElementById('footer-phone').textContent =
      data.footer.column3.details.hotline;
  if (document.getElementById('footer-address'))
    document.getElementById('footer-address').innerHTML = convertStr(
      data.footer.column3.details.address,
    );
...
function fetchNews() {
  const newsApi =
    'https://alpha-api.lotuslms.com/page/api/get-news-blogs?order_by=ts&order_direction=-1&page=1&items_per_page=5&submit=1&_sand_ajax=1&_sand_platform=3&_sand_readmin=1&_sand_is_wan=false&_sand_domain=olympictuoitho&_sand_use_internal_network=0&allow_cache_api_cdn=1';
  ...
  fetch(newsApi)
    ...
    .then((resData) => {
      ...
      const newsHtml = resData.result
        .slice(0, 5)
        .map((news) => {
          const date = new Date(news.ts * 1000).toLocaleDateString('vi-VN');
          const img =
            news.thumbnail || 'https://via.placeholder.com/600x400?text=News';
    ...
    .catch((err) => {
      ...
      document.getElementById('news-container').innerHTML =
        '<p class="col-span-full text-center text-slate-500 font-medium pb-8 pt-4">Không thể tải tin tức lúc này, vui lòng thử lại sau.</p>';
    });
}
...
function drawRoadmapRoad() {
  ...
  svg.innerHTML = `
        <defs>
            <linearGradient id="road-grad" x1="0" y1="0" x2="0" y2="1">
                <stop offset="0%" stop-color="${
                  isDark ? '#334155' : '#86efac'
                }"/>
                <stop offset="50%" stop-color="${
                  isDark ? '#1e3a5f' : '#7dd3fc'
                }"/>
    ...
    isDark ? 'rgba(250,250,250,0.2)' : 'rgba(255,255,255,0.9)'
  }" stroke-width="3" stroke-dasharray="14 10" stroke-linecap="round">
            <animate attributeName="stroke-dashoffset" from="0" to="-48" dur="1.5s" repeatCount="indefinite"/>
        </path>
        <!-- Sparkle dots -->
        <path d="${d}" fill="none" stroke="${
    isDark ? 'rgba(96,165,250,0.3)' : 'rgba(37,99,235,0.15)'
  }" stroke-width="2" stroke-dasharray="4 16" stroke-linecap="round">
...
(function initStatsAnimation() {
  function animateCountUp(el) {
    ...
    function tick(now) {
      ...
      el.textContent =
        (value >= 1000 ? Math.round(value / 1000) : value) + suffix;
    ...
    .map((item, idx) => {
      ...
      return `
        <div class="testimonial-card opacity-0 bg-white dark:bg-slate-800 rounded-2xl p-6 shadow-md border border-blue-50 dark:border-slate-700 flex flex-col gap-4 hover-card-lift transition-shadow"
             style="animation: testimonial-in 0.55s cubic-bezier(0.34,1.56,0.64,1) forwards; animation-delay: ${idx *
               0.1}s;">
            <!-- Stars -->
            <div class="text-amber-400 text-lg tracking-wide">${stars(
              item.rating,
            )}</div>
            <!-- Quote -->
            <p class="text-slate-600 dark:text-slate-300 text-sm leading-relaxed italic flex-1">
                "${convertStr(item.quote)}"
            </p>
            <!-- Author -->
      ...
      const observer = new IntersectionObserver(
        ([entry]) => {
          if (entry.isIntersecting) {
            card.style.animation = `testimonial-in 0.5s cubic-bezier(0.34,1.4,0.64,1) ${idx *
              0.08}s forwards`;
...
function vongThiOpenRegModal(date, session, status) {
  ...
  document.getElementById('modal-title').textContent = isActive
    ? '🎯 Xác nhận vào thi'
    : '🔔 Đặt thông báo';
  document
    .getElementById('modal-action-active')
    .classList.toggle('hidden', !isActive);
  document
    .getElementById('modal-action-notify')
    .classList.toggle('hidden', isActive);
  document
    .getElementById('modal-active-info')
    .classList.toggle('hidden', !isActive);
  document
    .getElementById('modal-notify-info')
    .classList.toggle('hidden', isActive);
  document.getElementById('modal-time-label').textContent = isActive
    ? 'Thời gian còn lại'
    : 'Thời gian bắt đầu';
  document.getElementById('modal-time-val').textContent = isActive
    ? '30 phút'
    : session.split('–')[0];
...
function initOlympicApp() {
  fetch('/o-assets/data.json')
    ...
    .then((data) => {
      ...
      if (
        document.getElementById('rules-title') &&
